...
const RARITY_STYLES: Record<Rarity, { border: string; text: string; bg: string; label: string }> = {
  common: { border: "border-gray-500/60", text: "text-gray-300", bg: "bg-gray-500/10", label: "Common" },
  uncommon: { border: "border-green-400/60", text: "text-green-300", bg: "bg-green-500/10", label: "Uncommon" },
  rare: { border: "border-blue-400/60", text: "text-blue-300", bg: "bg-blue-500/10", label: "Rare" },
  epic: { border: "border-purple-400/60", text: "text-purple-300", bg: "bg-purple-500/10", label: "Epic" },
  legendary: { border: "border-orange-400/80", text: "text-orange-300", bg: "bg-orange-500/10", label: "Legendary" }
};
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className={`shrink-0 text-[10px] font-black uppercase tracking-wider ${style.text}`}>{style.label}</span>
          </div>
          ...
          {!compact && item.passive && (
            <div className="mt-1 text-xs font-medium text-amber-300">✦ {item.passive.label}</div>
          )}
        </div>
      </div>
      {locked && (
